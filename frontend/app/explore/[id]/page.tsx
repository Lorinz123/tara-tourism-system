...
export default function DetailPage() {
  ...
  */
  ...
  const isMenu = rawId.startsWith('menu-');
  const isPlace = rawId.startsWith('place-');
  const actualId = rawId.split('-')[1];
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        */
        if (isMenu) {
          // FIXED: Changed hardcoded local URL string to dynamic configuration context entry point
          const itemRes = await fetch(`${API_URL}/api/menu-items/${actualId}`, {
          ...
          */
          // FIXED: Changed hardcoded local URL string to dynamic configuration context entry point
          const menuRes = await fetch(`${API_URL}/api/owner-menu/${data.user_id}`, {
            signal: controller.signal,
          });
        ...
        */
        if (isPlace) {
          // FIXED: Changed hardcoded local URL string to dynamic configuration context entry point
          const placeRes = await fetch(`${API_URL}/api/places/${actualId}`, {
            signal: controller.signal,
          });
    ...
    <div className="min-h-screen bg-[#FDFCF8] pb-20">
      ...
      <div className="relative h-[220px] md:h-[300px] w-full overflow-hidden z-0">
        <img
          src={
            isMenu
              ? item.owner?.hero_image || 'https://images.unsplash.com/photo-1555396273-367ea4eb4db5?q=80&w=2000'
              : item.image_url || 'https://images.unsplash.com/photo-1507525428034-b723cf961d3e?q=80&w=1200'
          }
          alt="Hero"
      ...
      <main className="max-w-5xl mx-auto px-4 md:px-6 -mt-16 md:-mt-24 relative z-10">
              ...
              <div className="flex flex-col lg:flex-row gap-6 justify-between items-start">
                <div className="space-y-4 min-w-0">
                  ...
                  <p className="text-base md:text-xl text-gray-700 italic font-medium">
                    "{item.owner?.tagline || 'No tagline available'}"
                  </p>

                  {item.owner?.announcement && (
              ...
              {menuItems.map((m: any) => (
                ...
                >
                  ...
                  <button
                    onClick={() => {
                      addToCart(m);
                      setNotification(`${m.name} added to cart!`);
                    }}
        ...
        {isPlace && (
          <div className="bg-white p-5 md:p-10 rounded-3xl shadow-2xl border border-gray-100">
            ...
            <div className="flex items-start gap-2 text-gray-600 mb-8 font-medium break-words">
              <MapPin size={20} className="text-orange-600" />
